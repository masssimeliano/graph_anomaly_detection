...
def create_metric_plot(
    ...
    for dataset in datasets:
        ...
        save_path = os.path.join(SAVE_DIR_OCGNN, f"{dataset}_{name}.png")
...
def plot_heatmap(metric_name: str, title: str, cmap: str = "viridis"):
    ...
    save_path = os.path.join(
        SAVE_DIR_OCGNN, f"heatmap_{metric_name}_epoch{target_epoch}.png"
    )
    plot.savefig(save_path, dpi=300)
    plot.show()
...
def make_epoch_pivot_table(
    ...
    save_csv_path = os.path.join(
        SAVE_DIR_OCGNN, f"pivot_metrics_epoch{target_epoch}.csv"
    )
